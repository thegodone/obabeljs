...
Molecule* Molecule::fromSmiles(string smiles) {
          ...
          return new Molecule(conv.ReadString(&mol, smiles)); // not sure it will work!
}
...
std::vector<unsigned int> Molecule::canonicalindex()
{
  ...
  OBGraphSym gs = OBGraphSym(&mol);
  std::vector<unsigned int> symclasses;
  gs.GetSymmetry(symclasses);

  std::vector<unsigned int> canonlabels;
  CanonicalLabels(&mol, symclasses, canonlabels); // need to add canon.h header to see this function properly.
...
std::vector<unsigned int> Molecule::Mol_NMR_FP()
{
          ...
          std::vector<unsigned int> fpNMR[mypat.size()];
          for (int i =0; i<mypat.size();i++){
            sp.Init(mypat[i]);
            // complete Matching 
            (bool) sp.Match(mol, false);
...
std::vector<unsigned int> Molecule::Atom_NMR_FP()
{
          ...
          std::vector<unsigned int> fpatomNMR[mypat.size()];
